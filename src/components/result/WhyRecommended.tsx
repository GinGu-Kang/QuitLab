    ...
    <div className="space-y-4">
      ...
      {results.map((result, index) => {
        ...
        return (
          <div
            key={result.item.id}
            className={`rounded-[18px] border bg-quiz-card p-5 ${index === 0 ? 'border-quiz-teal' : 'border-quiz-border'}`}
          >
            ...
            <div className="mt-4 rounded-[14px] bg-[rgba(13,148,136,0.08)] p-4 text-sm leading-7 text-quiz-text-secondary">{result.reason}</div>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {strengths.length ? strengths.map((entry) => (
                    <span key={`${result.item.id}_${entry.label}`} className="rounded-full border border-[rgba(16,185,129,0.28)] bg-[rgba(16,185,129,0.1)] px-3 py-1 text-xs text-green-200">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {weaknesses.length ? weaknesses.map((entry) => (
                    <span key={`${result.item.id}_weak_${entry.label}`} className="rounded-full border border-[rgba(245,158,11,0.28)] bg-[rgba(245,158,11,0.1)] px-3 py-1 text-xs text-yellow-200">
                      {entry.icon} {entry.label} (내 {entry.user} &lt; 필요 {entry.required})
                    </span>
